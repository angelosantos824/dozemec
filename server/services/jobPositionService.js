...
const repo = require("../repositories/jobPositionRepository");
...
async function get(tenantId, id) {
  ...
  if (!item) throw new AppError("Cargo nao encontrado", 404);
  return item;
}

async function create({ tenantId, userId, ipAddress, data }) {
  validators.catalog(data);
  return persist({ tenantId, userId, ipAddress, data, action: "job_position.create" });
}

async function update({ tenantId, id, userId, ipAddress, data }) {
  validators.catalog(data, true);
  return persist({ tenantId, id, userId, ipAddress, data, action: "job_position.update" });
}
...
async function remove({ tenantId, id, userId, ipAddress }) {
  ...
  try {
    ...
    if (await repo.countActiveEmployees(tenantId, id, connection)) throw new AppError("Nao e permitido excluir cargo com funcionarios ativos", 400);
    await repo.softDelete(tenantId, id, userId, connection);
    await auditRepository.create(connection, { tenantId, userId, action: "job_position.delete", entity: "job_positions", entityId: id, oldData, newData: { deleted: true }, ipAddress });
...
async function persist({ tenantId, id, userId, ipAddress, data, action }) {
  ...
  try {
    ...
    if (data.code) {
      ...
      if (duplicate && Number(duplicate.id) !== Number(id)) throw new AppError("Ja existe cargo com este codigo nesta empresa", 409);
    }
    ...
    await auditRepository.create(connection, { tenantId, userId, action, entity: "job_positions", entityId: itemId, oldData, newData, ipAddress });
    await connection.commit();
    return newData;
  } catch (error) {
    await connection.rollback();
